...
const RefineImageCanvas: React.FC<RefineImageCanvasProps> = ({ imageUrl, setIsPromptModalOpen, editInspectorMinimized = false, onDownload, onOpenGallery }) => {
  ...
  useEffect(() => {
    console.log('🖼️ RefineImageCanvas: imageUrl changed:', imageUrl);
    if (imageUrl) {
      const img = new Image();
      img.onload = () => {
        console.log('🖼️ RefineImageCanvas: Image loaded successfully');
        setImage(img);
      ...
      img.onerror = (error) => {
        console.error('🖼️ RefineImageCanvas: Failed to load image:', error);
      };
      img.src = imageUrl;
    } else {
      console.log('🖼️ RefineImageCanvas: No imageUrl provided, clearing image');
      setImage(null);
