...
import { setSelectedMaskId, setMaskInput, updateMaskStyle, updateMaskStyleLocal, getMasks } from "@/features/masks/maskSlice";
...
export default function RegionsWrapper() {
  ...
  useEffect(() => {
    if (maskStatus === 'completed' && displayMasks.length === 0 && inputImageId) {
      console.log('🔄 Loading masks for completed status, inputImageId:', inputImageId);
  ...
  if (displayMasks.length === 0 && maskStatus !== 'processing' && maskStatus !== 'completed') {
    return null;
  }

  // Show placeholder regions when processing/completed but masks not loaded yet
  const isLoading = displayMasks.length === 0 && (maskStatus === 'processing' || maskStatus === 'completed' || isRegionExtractionProcessing);
  const regionsToShow: (typeof displayMasks[0] | { id: string; index: number })[] = isLoading 
    ? Array.from({ length: 4 }, (_, i) => ({ id: `placeholder-${i}`, index: i })) 
    : displayMasks;
    ...
    <div className="relative space-y-3 py-1 w-full max-w-sm">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-3">
          ...
          const isPlaceholder = isLoading && 'index' in region && typeof region.id === 'string' && region.id.startsWith('placeholder');
          const maskRegion = isPlaceholder ? null : (region as typeof displayMasks[0]);
          ...
          const isSelected = maskRegion ? selectedMaskId === maskRegion.id : false;
          const regionIndex = isPlaceholder ? (region as { id: string; index: number }).index : index;

          return (
            <div
              key={isPlaceholder ? `placeholder-${regionIndex}` : maskRegion!.id}
              className="flex flex-col gap-1.5 group"
              onDragOver={isPlaceholder ? undefined : handleDragOver}
              onDrop={isPlaceholder ? undefined : (e) => handleDrop(e, maskRegion!.id)}
            >
              {/* Region Label */}
              ...
              <div className="flex items-center gap-2">
                ...
                <div
                  ...
                  onClick={isPlaceholder ? undefined : () => handleMaskClick(maskRegion!.id)}
                >
                {isPlaceholder ? (
                ...
                <div className="flex-1 min-w-0">
                  {isPlaceholder ? (
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 text-sm border border-gray-200 rounded focus:outline-none focus:border-primary-500 bg-white shadow-sm"
                      placeholder="Type or drag from catalog"
                    />
                  )}
                </div>
        ...
        {/* Regions Preview Canvas */}
        {baseImageUrl && !isLoading && displayMasks.length > 0 && (
          <div className="mt-3">
            <div className="relative w-full aspect-square rounded-lg border border-gray-200 overflow-hidden bg-white">
              <canvas
