...
const MaterialCustomizationSettingsCompact: React.FC = () => {
  ...
  const { selections, availableOptions, selectedStyle, inputImageId } = useAppSelector(
  ...
  const getInitialSections = () => {
    if (selectedStyle === "photorealistic") {
      return ["type", "walls", "floors", "context", "style", "weather", "lighting"];
    } else {
  ...
  useEffect(() => {
    ...
    if (newSections.length > 0 && !newSections.includes(expandedSection || "")) {
          ...
          <div className="grid grid-cols-12 gap-1 max-h-">
            ...
            >
              ...
              <span className="text-[9px] font-medium text-center text-gray-600 line-clamp-1 w-full">
                {uploading || inputImagesLoading ? "Uploading..." : "Upload"}
              </span>
              <input
                ref={fileInputRef}
                type="file"
      ...
      {selectedStyle === "photorealistic" ? (
        ...
        <>
          {Object.entries(currentData).map(([subcategoryKey]) => (
            ...
            >
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-1">
                {(currentData[subcategoryKey] as any[])?.map((option: any) => (
                  <CategorySelectorCompact
                    key={option.id}
                    title={option.displayName}
